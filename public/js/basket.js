...
$("#checkout_btn").click(function() {
    ...
    var attempt=ar.substring(pos,ar.length-1);
    var manip=attempt.substring(0,attempt.indexOf("/"));
    var dec = window.atob(manip);
    var dec2 = dec.split(" ")[0];
    const url = '/'+ manip+"/login=true/checkout/"+dec2;
...
$("#Search").on('click',function(){
    ...
    function clickRemove(id,ht){
        ...
        var attempt=ar.substring(pos,ar.length-1);
        var manip=attempt.substring(0,attempt.indexOf("/"));
        newUrl = '/BasketOps/'+ id+'/' + ht+'/m';
        const Http= new XMLHttpRequest();
        Http.open("GET",newUrl);
        Http.send();  
        var timer= setTimeout(function(){$(location).attr('href','/' +manip+'/login=true/Basket');},500)
    };
