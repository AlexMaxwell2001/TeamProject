    ...
    document.getElementById('stylB').disabled="true";
    document.getElementById('stylB').style.cursor="default";
    ...
    $(document).ready(function(){
        ...
        if(ar.includes('true')){
            ...
            document.getElementById('theH').style.visibility="visible"
            document.getElementById('logWarning').style.visibility="hidden"
        }
    })
    ...
    $("#listSizes").on('change',function(){
        const article = document.querySelector('#theH');
        var value = document.getElementById("listSizes").value;
        if(value!= "unselected"){
            article.dataset.size=value;
            document.getElementById('stylB').style.background="green";
            document.getElementById('stylB').style.border="green";
            document.getElementById('stylB').style.color="white";
            document.getElementById('stylB').disabled=false;
            document.getElementById('stylB').style.cursor="pointer";
        }else{
            document.getElementById('stylB').disabled=true; 
            document.getElementById('stylB').style.background="rgb(231, 238, 236)";
            document.getElementById('stylB').style.border="rgb(231, 238, 236)";
            document.getElementById('stylB').style.color="rgb(137, 152, 155)";
            document.getElementById('stylB').style.cursor="default";
        }
    });

    $("#theH").on('click',function(){
        const article = document.querySelector('#theH');
        var id=article.dataset.id;
        var size=article.dataset.size;
        var newUrl;
        var change;
        var ar= current.href;
        var count=0;
        ...
        var attempt=ar.substring(pos,ar.length-1);
        var manip=attempt.substring(0,attempt.indexOf("/"));
        newUrl = '/'+ manip+'/login=true/basket/'+ id+'/' + size; 
        change='/'+ manip+'/login=true/basket';
        const Http= new XMLHttpRequest();
        const url= newUrl
        Http.open("GET",url);
        Http.send();
        var timer = setTimeout(function(){$(location).attr("href",change)},400);  
    });
    ...
    $(document).ready(function(){
        const select = document.querySelector('#listSizes');
        const name = document.querySelector('#pSize');
        var option;
        var sizes=name.dataset.sizes;
        var ar = sizes.split(",");
        for(var i=0;i<ar.length;i++){
            option = new Option(ar[i], ar[i]);
