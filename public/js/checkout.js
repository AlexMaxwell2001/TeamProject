    ...
    $("#goConfirm").on('click',function() {
        ...
        var val2=document.getElementById("finalP").innerHTML.replace("€","");
        var ar= current.href;
        var count=0;
        var pos=0;
        for(var i=0;i<ar.length;i++){
        ...
        const url= '/BasketOps/'+val2 + '/s/m/k';
        Http.open("GET",url);
        Http.send(); 
        var timer= setTimeout(function(){$(location).attr('href', '/'+ manip+'/login=true/Confirmation/'+ val);},1000);
    });

    $(document).ready(function(){
        var val = document.getElementById("teller").innerHTML;
        var ar= current.href;
        var count=0;
        var pos=0;
        for(var i=0;i<ar.length;i++){
    ...
    $(document).ready(function(){
        var ar = current.href;
        var val=document.getElementById("finalvalue").innerHTML.toString().replace("€","").replace("<strong>","").replace("</strong>","");
        if(val==0){
            var count=0;
            var pos=0;
            for(var i=0;i<ar.length;i++){
    ...
    $("#country").on('change',function(){
       var ar= document.getElementsByClassName('form-control');
        var temp=document.getElementById("country").value;
        var vis= document.getElementById("warning").style.visibility;
        var flag=false;
        for(var i=0;i<ar.length;i++)
        {
            if(ar[i].value.length>0){}
            else{flag=true;}
        }
        if(temp == "unselected")flag=true;
        if(!flag && vis === "hidden"){
                document.getElementById('goConfirm').disabled=false;
                document.getElementById('goConfirm').style.cursor="cursor";
        }else{
            document.getElementById('goConfirm').disabled=true;
                document.getElementById('goConfirm').style.cursor="default";
        } 
    })


    //The user can't submit their registration with the necessary fields being filled in
    $(".form-control").on('keyup',function() {
        ...
        $("#cc-number").on('keyup',function() {
            var val2= document.getElementById("cc-number").value.split("-").join("");
            var modified= document.getElementById("cc-number").value;
            if (val2.length > 0) {
                val2 = val2.match(new RegExp('.{1,4}', 'g')).join("-");
                document.getElementById("cc-number").value= val2;
            }
            else if(val2.length==0){
                document.getElementById("warning").style.visibility="hidden";
            }
            if(!validateCard(modified)){
                document.getElementById('goConfirm').disabled=true;
                document.getElementById('goConfirm').style.cursor="default";
...
function validateCard(num) {
    var sum = 0;
    var cardNumber = num.toString().replaceAll("-","");
    sum=0;
     for(var j=cardNumber.length-1;j>=0;j--)
    {
        var number=parseInt(""+cardNumber.charAt(j));
    ...
    if(cardNumber.length<14||cardNumber.length>19)return false;
    return sum%10==0;
}
...
$("#Search").on('click',function(){
    ...
    $("#cc-expiration").on("keyup",function(){
        var val=document.getElementById("cc-expiration").value;
        if(val.length==2){ }
        if(lastval<=1){
            if(val.length==2){
                document.getElementById("cc-expiration").value=val + "/";
                lastval= val.length;
            }
        }
    })
    ...
    function findSpecific(value){
        ...
        var manip=attempt.substring(0,attempt.indexOf("/"));
            if(n=="T-shirts"){
                n="t-shirt";
                $(location).attr('href', '/' +manip+'/login=true/Products/ProductName/'+n);
            }
            else if(n=="Bags"){
                n="bag";
                $(location).attr('href', '/' +manip+'/login=true/Products/ProductName/'+n);
                ...
                n="coat";$(location).attr('href', '/' +manip+'/login=true/Products/ProductName/'+n);
            }
            else if(n=="Jeans"){
                n="jean";
                $(location).attr('href', '/' +manip+'/login=true/Products/ProductName/'+n);
            ...
            else $(location).attr('href','/' +manip+'/login=true/Products/ProductName/'+n+'/fullview/'+n);
    };

    $("#redeeming").on('click',function(){
        var val= document.getElementById("codePromo").value;
        var val2=document.getElementById("finalP").innerHTML.replace("€","");
        const article = document.querySelector('#redeeming');
        var id=article.dataset.used;
        if(val=="CS353"){
            if(id!= "used"){
                document.getElementById("finalP").innerHTML="€"+Math.floor(val2*0.8);
                document.getElementById("warningNo").style.visibility="hidden";
                article.dataset.used="used";
                const Http= new XMLHttpRequest();
                const url= '/BasketOps/'+val2 + '/s/m/k';
                Http.open("GET",url);
                Http.send(); 
            }else{
                document.getElementById("warningUsed").style.visibility="visible";
            }
        }else{
            document.getElementById("warningNo").style.visibility="visible";
            document.getElementById("warningUsed").style.visibility="hidden";
        }
    })
